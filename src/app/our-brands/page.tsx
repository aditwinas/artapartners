...
export default async function Brands() {
  ...
  return <Shell content={c}><PageHero label="OUR BRANDS" title={c.brandsTitle} description={c.brandsIntro} />
    ...
    <div className="container brand-details">{c.brands.map((b, i) => <section className="brand-detail" id={b.slug} key={b.slug}><BrandVisual brand={b} index={i} priority={i === 0} /><div className="brand-detail-copy"><p className="eyebrow">{b.category}</p><h2>{b.name}</h2><h3>{b.headline}</h3><p>{b.description}</p>{(b.city || b.founded) && <dl className="brand-facts">{b.city && <div><dt>Berawal di</dt><dd>{b.city}</dd></div>}{b.founded && <div><dt>Berdiri sejak</dt><dd>{b.founded}</dd></div>}</dl>}{b.url && <TextLink href={b.url}>Kunjungi {b.name}</TextLink>}</div></section>)}</div>
  </Shell>;
}
