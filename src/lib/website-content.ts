export type Brand = { slug: string; name: string; category: string; headline: string; description: string; city: string; founded: string; image: string; imageAlt: string; url: string; workOn: string; summary: string };
...
// Copy from WEBSITE PAGE: REVISI takes precedence; unknown facts stay unpublished.
export const defaultWebsiteContent: WebsiteContent = {
  ...
  steps: [
    { title: 'Identifikasi', description: 'Kenali bisnisnya lebih dalam. Kami melihat lebih dekat kondisi keuangan, produk, pasar, dan operasional untuk memahami apa yang sudah kuat, apa yang perlu dibenahi, dan di mana peluangnya.', detail: 'Kami mulai dari memahami bisnis. Melihat kondisi bisnis, produk, pasar, operasional, dan peluang yang ada untuk menentukan area yang paling perlu dikembangkan.' },
    { title: 'Kemitraan', description: 'Tumbuh dengan partner yang tepat. Kami menyelaraskan visi dengan pemilik brand dan membangun kemitraan yang transparan, dengan tujuan dan peran yang jelas sejak awal.', detail: 'Kami menyusun langkah bersama. Menyelaraskan visi, strategi, peran, dan kebutuhan agar setiap keputusan punya arah yang jelas.' },
    { title: 'Akselerasi', description: 'Dari rencana, jadi langkah nyata. Setelah fondasinya siap, kami membantu mengembangkan brand melalui strategi, pengelolaan operasional, inovasi, dan pendanaan yang sesuai dengan kebutuhan bisnis.', detail: 'Kami ikut mendorong eksekusinya. Dari strategi hingga implementasi, kami membantu bisnis bergerak lebih cepat dan membuka peluang pertumbuhan berikutnya.' },
  ],
...
export function mergeWebsiteContent(incoming: unknown): WebsiteContent {
  const merge = (base: unknown, value: unknown): unknown => {
    ...
    if (Array.isArray(base)) {
      ...
      const sample = base === defaultWebsiteContent.brands ? { ...base[0], city: '', founded: '', workOn: '', summary: '' } : base[0];
